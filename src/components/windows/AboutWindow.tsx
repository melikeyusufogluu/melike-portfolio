...
export default function AboutWindow() {
  ...
  const [openInterest, setOpenInterest] = useState<string | null>(interests[0]?.title ?? null)
    ...
    <div className="flex flex-col gap-3">
      <div className="flex gap-3 items-start">
        ...
        <div>
          ...
          <p>{profile.title} — {profile.tagline}</p>
      ...
      <div className="bevel-in bg-white p-2 min-h-[160px]">
        {tab === 'profile' && (
          <p className="font-mono text-[12px] whitespace-pre-wrap">{profile.summary}</p>
        )}

        {tab === 'education' && (
          <div className="text-[12px]">
            <p>
              <span className="font-bold">{education.school}</span> — {education.degree}
            </p>
            ...
            {interests.map((i) => {
              ...
              return (
                <div key={i.title} className="bevel-out bg-win-gray">
                  ...
                  {isOpen && (
                    <div className="bevel-in bg-white m-1 p-2 text-[12px]">{i.description}</div>
                  )}
                </div>
              )
            })}
